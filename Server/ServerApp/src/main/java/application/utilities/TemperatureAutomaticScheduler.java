...
public class TemperatureAutomaticScheduler {
	...
	private List<ScheduleDetail> getScheduleDetailsFromStateChanges(Iterable<StateChange> messages) {
		...
		for (int hourOfWeek = 0; hourOfWeek <= 167; hourOfWeek++) {
			...
			if (occurences == 0) {
				...
				for (int hourOfWeekPrevious = hourOfWeek - 1; hourOfWeekPrevious >= 0; hourOfWeekPrevious--) {
					Integer previousValue = scheduleValues.get(hourOfWeekPrevious / 7).get(hourOfWeekPrevious % 24)
							.getLeft();
					Integer previousOccurences = scheduleValues.get(hourOfWeekPrevious / 7).get(hourOfWeekPrevious % 24)
							.getRight();
		...
		// for (int hourOfWeek = getHourOfWeek(stateChange); hourOfWeek <= 167;
		// hourOfWeek++) {
		// Integer value =
		// scheduleValues.get(hourOfWeek/7).get(hourOfWeek%24).getLeft();
		// Integer occurences =
		// scheduleValues.get(hourOfWeek/7).get(hourOfWeek%24).getRight();
		//
		// value = value +
		// Integer.valueOf(stateChange.getValues().get(SUPPORTED_VALUE));
		// occurences = occurences+1;
		//
		...
		// for (int hourOfWeek = 0; hourOfWeek < getHourOfWeek(stateChange);
		// hourOfWeek++) {
		// Integer value =
		// scheduleValues.get(hourOfWeek/7).get(hourOfWeek%24).getLeft();
		// Integer occurences =
